"use strict";
/* 넘버원 김포B 공비 - 관리자 하단 데이터 상태 새로고침 복원판 20260716-13 */
...
function initializeAdminButton() {
    ...
    if (elements.adminBottomRefreshBtn) elements.adminBottomRefreshBtn.addEventListener("click", () => loadAdminDashboard(true));
...
loadAdminDashboard = function loadAdminDashboardV23(showLoading = true) {
    ...
    const bottomButton = elements.adminBottomRefreshBtn;
    state.adminLoading = true;
    if (button) { button.disabled = true; button.textContent = "불러오는 중…"; }
    if (bottomButton) { bottomButton.disabled = true; bottomButton.textContent = "새로고침 중…"; }
    if (showLoading) {
    ...
    const task = (async () => {
        try {
            ...
            if (bottomButton) { bottomButton.disabled = false; bottomButton.textContent = "데이터 상태 새로고침"; }
        }
    })();
