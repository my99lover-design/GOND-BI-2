"use strict";
/* 넘버원 김포B 공비 - 관리자 현재상태 느낌표 오탐 보정 20260716-38 */
...
const acceptanceCounterState = { accepted: 0, rejected: 0 };
document.addEventListener("DOMContentLoaded", initializeApp);
async function initializeApp() {
    initializeTheme();
    initializeAcceptanceCounter();
...
function applyTheme(theme) {
    ...
    if (elements.themeToggle) elements.themeToggle.textContent = normalizedTheme === "dark" ? "☀️ 밝기" : "🌙 밝기";
    const themeColor = document.querySelector('meta[name="theme-color"]');
    if (themeColor) themeColor.setAttribute("content", normalizedTheme === "dark" ? "#08111f" : "#e9eef5");
}
...
function loadAcceptanceCounter() {
    try {
        ...
        return { accepted: saved?.accepted, rejected: saved?.rejected };
    } catch (error) {
...
function changeAcceptanceCount(type, delta) {
    ...
    if (next === current) {
        if (delta < 0) showToast("0건 아래로 줄일 수 없습니다.");
        return;
    }
...
function resetAcceptanceCounter() {
    ...
    if (!window.confirm("수락·거절 카운터를 모두 0건으로 초기화할까요?")) return;
    acceptanceCounterState.accepted = 0;
    acceptanceCounterState.rejected = 0;
    saveAcceptanceCounter();
    renderAcceptanceCounter();
    showToast("✅ 수락·거절 카운터를 초기화했습니다.");
}
function calculateAcceptanceCounter(accepted, rejected) {
    ...
    const rejectAvailable = Math.max(0, Math.floor(safeAccepted / 4) - safeRejected);
    const acceptedNeeded = Math.max(0, safeRejected * 4 - safeAccepted);
    return { total, acceptRate, rejectAvailable, acceptedNeeded };
}
function renderAcceptanceCounter() {
    ...
    elements.acceptanceAcceptRate.textContent = `${calculated.acceptRate.toFixed(1)}%`;
    elements.acceptanceRejectAvailable.textContent = `${calculated.rejectAvailable}건`;
    elements.acceptanceTargetAccepted.textContent = calculated.total === 0 ? "-" : calculated.acceptedNeeded > 0 ? `+${calculated.acceptedNeeded}건` : "달성";
    ...
    elements.acceptanceCounter.setAttribute("aria-label", `수락 ${accepted}건, 거절 ${rejected}건, 수락률 ${calculated.acceptRate.toFixed(1)}퍼센트, 추가 거절 가능 ${calculated.rejectAvailable}건`);
}
/* ========================= 앱 설치 ========================= */
...
function initializeHomeDataStatusCard() {
    const button = elements.homeDataUpdateBtn;
...
async function handleHomeDataRefresh() {
    if (navigator.onLine === false) {
        updateDataSyncStatus("offline", state.lastSuccessfulSyncAt);
        showToast("인터넷 연결 후 다시 업데이트해주세요.");
        return;
    }
    if (state.networkLoading) return;
    updateHomeDataStatusCard();
    const needsIntegrityRepair = typeof loadDataIntegrityResult === "function" && loadDataIntegrityResult()?.status === "failed";
    await refreshRecordsFromServer(true);
    let integrityResult = { success: true, skipped: true };
    if (needsIntegrityRepair && state.dataSyncState === "current" && state.records.length > 0) {
        integrityResult = await runDailyDataIntegrityValidation(true);
    }
    updateHomeDataStatusCard();
    if (state.dataSyncState === "current" && integrityResult?.success !== false) showToast("✅ 데이터 업데이트를 완료했습니다.");
    else showToast("⚠ 데이터 업데이트를 완료하지 못했습니다.");
}
function updateHomeDataStatusCard() {
    const slot = elements.homeDataUpdateSlot;
    const button = elements.homeDataUpdateBtn;
    if (!slot || !button) return;

    const isHome = state.view === "regions" && !document.body.classList.contains("number-one-screen-active");
    const count = Array.isArray(state.records) ? state.records.length : 0;
    const status = cleanText(state.dataSyncState) || "checking";
    const integrity = typeof loadDataIntegrityResult === "function" ? loadDataIntegrityResult() : null;
    const integrityFailed = integrity?.status === "failed";
    const noUsableData = count === 0 && !state.loading;
    const syncFailed = status === "error";
    const shouldShow = noUsableData || syncFailed || integrityFailed;

    slot.hidden = !isHome || !shouldShow;
    if (slot.hidden) {
        button.disabled = false;
        button.className = "home-data-update-btn";
...
function renderCurrentView() {
    ...
    elements.buttonGrid.classList.toggle("dong-grid", state.view === "dongs");
...
function updateHeaderAndNavigation() {
    ...
    if (isHome) {
        ...
        if (elements.compactGpsSection) elements.compactGpsSection.hidden = true;
    } else {
        ...
        if (elements.compactGpsSection) elements.compactGpsSection.hidden = !["apartments", "dongs", "cards"].includes(state.view);
    }
    updateInstallButtonVisibility();
    if (elements.homeSupportSection) elements.homeSupportSection.hidden = !isHome;
    if (elements.homeBottomActions) elements.homeBottomActions.hidden = !isHome;
    if (elements.acceptanceCounter) elements.acceptanceCounter.hidden = !isHome;
    if (elements.numberOneOpenBtn) elements.numberOneOpenBtn.hidden = !isHome;
...
function initializeAdminButton() {
    ...
    document.querySelectorAll("#adminMenu [data-admin-view]").forEach(button => {
        button.addEventListener("click", () => setAdminView(button.dataset.adminView));
    });
    if (elements.adminMenuBackBtn) elements.adminMenuBackBtn.addEventListener("click", () => setAdminView("menu"));
    ...
    if (elements.normalizeApartmentFormatsBtn) elements.normalizeApartmentFormatsBtn.addEventListener("click", () => runPasswordCleanup("apartmentFormat"));
}
...
function openAdminDashboardModal() { setAdminView("menu"); openModal(elements.adminModal); loadAdminDashboard(true); }
...
async function authenticateAdmin(pin) {
    ...
    try {
        ...
        if (elements.adminBtn) {
            ...
            if (label) label.textContent = "🔒 관리자";
            else elements.adminBtn.textContent = "🔒 관리자";
            updateAdminButtonVisibility();
        }
        if (elements.adminPinSubmitBtn) {
    ...
    return {
        ...
        dataQuality: {
            ...
            cleanup: {
                ...
                apartmentFormatRowCount: Number(data?.dataQuality?.cleanup?.apartmentFormatRowCount) || 0,
                apartmentFormatPlaceCount: Number(data?.dataQuality?.cleanup?.apartmentFormatPlaceCount) || 0
            },
...
function setAdminView(view) {
    const views = Object.freeze({
        menu: { title: "🛠 관리자 메뉴", panel: null },
        current: { title: "현재 상태", panel: elements.adminCurrentPanel },
        performance: { title: "앱 성능 측정", panel: elements.adminPerformancePanel },
        quality: { title: "데이터 오류 점검", panel: elements.adminDataQualityPanel },
        diagnostics: { title: "앱 진단 보고서", panel: elements.adminDiagnosticsPanel },
        backup: { title: "백업·복구", panel: elements.adminBackupPanel },
        stats: { title: "통계", panel: elements.adminStatsPanel }
    });
    const target = Object.prototype.hasOwnProperty.call(views, view) ? view : "menu";
    state.adminView = target;
    if (elements.adminMenu) elements.adminMenu.hidden = target !== "menu";
    const panels = [
        elements.adminCurrentPanel,
        elements.adminPerformancePanel,
        elements.adminDataQualityPanel,
        elements.adminDiagnosticsPanel,
        elements.adminBackupPanel,
        elements.adminStatsPanel
    ];
    for (const panel of panels) if (panel) panel.hidden = panel !== views[target].panel;
    if (elements.adminMenuBackBtn) elements.adminMenuBackBtn.hidden = target === "menu";
    if (elements.adminModalTitle) elements.adminModalTitle.textContent = views[target].title;
    if (elements.adminContent) elements.adminContent.scrollTop = 0;
    if (target === "diagnostics" && state.adminDashboard) renderAdminDiagnostics(false);
    if (target === "performance" && state.adminDashboard) renderAdminPerformanceReport();
}
...
function renderPasswordCleanupActions(cleanup, pendingCount) {
    ...
    const apartmentFormatRowCount = Number(cleanup?.apartmentFormatRowCount) || 0;
    const apartmentFormatPlaceCount = Number(cleanup?.apartmentFormatPlaceCount) || 0;
    ...
    if (elements.normalizeApartmentFormatsBtn) {
        elements.normalizeApartmentFormatsBtn.disabled = busy;
        elements.normalizeApartmentFormatsBtn.textContent = state.passwordCleanupMode === "apartmentFormat"
            ? "형식 정리중…"
            : apartmentFormatRowCount > 0
                ? `아파트별 형식 정리 (${apartmentFormatRowCount.toLocaleString()}행)`
                : "아파트별 형식 정리";
        elements.normalizeApartmentFormatsBtn.title = apartmentFormatRowCount > 0
            ? `${apartmentFormatPlaceCount.toLocaleString()}곳 · ${apartmentFormatRowCount.toLocaleString()}개 행이 대표 형식과 다릅니다.`
            : "아파트별 대표 형식을 다시 확인합니다.";
    }
}
...
async function runPasswordCleanup(mode) {
    ...
    const isApartmentFormat = mode === "apartmentFormat";
    const cleanup = state.adminDashboard.dataQuality.cleanup || {};
    const count = isSort
        ? Number(cleanup.sortableCount) || 0
        : isApartmentFormat
            ? Number(cleanup.apartmentFormatRowCount) || 0
            : Number(cleanup.duplicateCount) || 0;
    const title = isSort
        ? "쉬운 번호 우선·호수 정렬"
        : isApartmentFormat
            ? "아파트별 비밀번호 형식 정리"
    ...
    const detail = isSort
        ...
        : isApartmentFormat
            ? "각 아파트에서 가장 많이 쓰인 대표 형식으로 인식 가능한 비밀번호만 통일합니다. 호수와 비밀번호 숫자는 그대로 유지하고, 형식 동률·판단 불가 값은 변경하지 않습니다."
...
/* ========================= 비밀번호 추가·수정·삭제 ========================= */
function getPasswordTemplateScopeRecords(record) {
    if (!record) return [];
    const region = cleanText(record.region);
    const apartment = cleanText(record.apartment);
    if (isOfficeRecord(record)) {
        return state.indexes.recordsByApartmentDong.get(makeKey(region, apartment, normalizeDongValue(record.dong))) || [];
    }
    return state.indexes.recordsByApartment.get(makeKey(region, apartment)) || [];
}
function detectApartmentPasswordTemplate(record) {
    const passwords = [];
    for (const scopeRecord of getPasswordTemplateScopeRecords(record)) {
        passwords.push(...splitPasswords(scopeRecord.password));
    }
    return detectPasswordTemplate(passwords);
}
function openAddPwdModal(rowId) {
    ...
    const scopeRecords = getPasswordTemplateScopeRecords(record);
    const scopePasswords = scopeRecords.flatMap(item => splitPasswords(item.password));
    const detectedTemplate = detectApartmentPasswordTemplate(record);
    state.addPasswordTemplate = detectedTemplate;

    if (scopePasswords.length === 0) {
        elements.addPwdFormatStatus.textContent = "이 아파트의 첫 비밀번호는 호수·비밀번호·종료문자를 포함한 전체 형식으로 입력해주세요.";
        setAddPasswordMode("direct");
    } else if (detectedTemplate) {
        const scopeLabel = isOfficeRecord(record) ? "건물" : "아파트";
        elements.addPwdFormatStatus.textContent = `${scopeLabel}에서 가장 많이 쓰인 형식을 자동으로 적용합니다.`;
        elements.addPwdFormatSample.textContent = `대표 형식: ${detectedTemplate.sample} · ${detectedTemplate.count}건 기준`;
        setAddPasswordMode("smart");
    } else {
        elements.addPwdFormatStatus.textContent = "이 아파트의 기존 형식에서 숫자 위치를 확실히 판단할 수 없어 전체 입력 방식으로 열었습니다.";
        setAddPasswordMode("direct");
    }
...
function initializeGpsEvents() {
    ...
    if (elements.compactGpsRefreshBtn) elements.compactGpsRefreshBtn.addEventListener("click", requestManualGpsRefresh);
...
function updateGpsRefreshButton() {
    ...
    if (elements.compactGpsRefreshBtn) {
        elements.compactGpsRefreshBtn.disabled = state.gpsRefreshInProgress;
        elements.compactGpsRefreshBtn.textContent = state.gpsRefreshInProgress ? "갱신 중…" : "갱신";
        elements.compactGpsRefreshBtn.setAttribute("aria-busy", state.gpsRefreshInProgress ? "true" : "false");
    }
}
...
function renderCompactGpsButtons() {
    if (!elements.compactGpsSection || !elements.compactGpsButtons || elements.compactGpsSection.hidden) return;
    const container = elements.compactGpsButtons;
    container.replaceChildren();
    let placeholder = "";
    let items = [];
    if (!state.currentLocation) placeholder = "위치 확인 중";
    else if (state.records.length === 0) placeholder = "데이터 확인 중";
    else if (!state.locationsLoaded) placeholder = "좌표 확인 중";
    else if (state.locationsError || state.locationMap.size === 0) placeholder = "좌표 오류";
    else {
        items = getNearbyApartments(state.currentLocation.latitude, state.currentLocation.longitude, APP_CONFIG.GPS_BUTTON_COUNT);
        if (!items.length) placeholder = "근처 정보 없음";
    }
    if (placeholder) {
    ...
    while (container.children.length < APP_CONFIG.GPS_BUTTON_COUNT) {
        const button = document.createElement("button");
        button.type = "button";
        button.className = "compact-gps-btn";
        button.disabled = true;
...
const DIAGNOSTIC_APP_SHELL = Object.freeze([
    "./", "./index.html", "./style.css?v=20260716-37", "./number-one.css?v=20260716-36", "./script.js?v=20260716-38", "./number-one.js?v=20260716-36", "./manifest.json",
...
/* ========================= 성능 판정 현실화 v24 ========================= */
const FINAL_BUILD_INFO = Object.freeze({ fileVersion: "20260716-38", serviceWorkerVersion: "v63" });
...
const V25_UI_CONFIG = Object.freeze({
    fileVersion: "20260716-38",
    serviceWorkerVersion: "v63",
...
function updateAdminMenuIssueIndicators() {
    ...
    // 현재 상태는 즉시 조치가 필요한 운영 상태만 표시합니다.
    // 빈 비밀번호·GPS 미연결은 데이터 오류 점검 메뉴에서만 안내합니다.
